...
- `detailed` (padrão) — além das estatísticas, quebra por categoria, e desde 2026-08-10 a **lista "Despesas do mês"** (cada despesa do mês: nome, categoria, tipo + quando, valor — ordenada por valor decrescente, com nomes/categorias escapados). Os **gráficos** aparecem no **fim do email** (desde 2026-08-21): donut de distribuição por categoria como **PNG gerado pelo QuickChart.io e embutido inline via CID** (`<img src="cid:donut-chart">`; a parte MIME viaja dentro do próprio email com `contentDisposition: inline`, não aparece na lista de anexos e funciona em todos os clientes — substituiu a data URI base64, que o Gmail/Outlook bloqueavam, e o `conic-gradient`, sem suporte nos mesmos) + barra "Recorrentes vs únicas". Se o QuickChart falhar, fallback para barras horizontais "Despesas por categoria" + barra split.
...
- O email inclui um **PDF em anexo** (`relatorio-AAAA-MM.pdf`), gerado com `pdf-lib` (via `esm.sh`, corre nativamente em Deno). **Desde 2026-08-21 o PDF é idêntico ao que o utilizador vê no email**: mesmas secções pela mesma ordem — cabeçalho, estatísticas, despesas do mês, donut centrado (a mesma imagem PNG do QuickChart, embutida com `embedPng`) e barra "Recorrentes vs únicas" — removidas as barras por categoria e a legenda própria que existiam só no PDF.
