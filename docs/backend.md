...
| `svg-to-png` | 1 | true | `deno.json` | Conversão SVG → PNG (resvg-wasm); payload `{ svg, width, height }` |
...
`svg-to-png` (deployado v1, 2026-08-13) converte SVG → PNG com `@resvg/resvg-wasm` (import map `deno.json`, `npm:@resvg/resvg-wasm@2.6.2`; wasm `index_bg.wasm` + fonte NanumGothic Regular carregadas do jsDelivr). Render: `fitTo` zoom x2 por defeito; se `width` e `height` forem enviados, passa a `fitTo: width`. `svg` ausente (400), >200 000 chars (400 "SVG demasiado grande (max 200KB)"), erros → 500. Resposta `image/png` com `Cache-Control: public, max-age=3600`. `verify_jwt: true`.
...
- `detailed` (padrão) — além das estatísticas, quebra por categoria, e desde 2026-08-10 a **lista "Despesas do mês"** (cada despesa do mês: nome, categoria, tipo + quando, valor — ordenada por valor decrescente, com nomes/categorias escapados). Os **gráficos** aparecem no **fim do email** (desde 2026-08-21): donut de distribuição por categoria como **PNG gerado pelo QuickChart.io (Chart.js v4, cantos arredondados, sem legenda na imagem) e hospedado num URL público do Supabase Storage** — bucket `report-charts` (a função cria-o em runtime se não existir; migration `008_report_charts_bucket.sql` como registo), path `donuts/{user_id}-{AAAA-MM}.png`, upload com service role + `upsert:true` e `cacheControl 86400` → URL estável/cacheável usado no `<img src>`. A imagem nem viaja no email → **o único anexo é o PDF** (o CID inline anterior era listado pelo Gmail como anexo fantasma "attachment-1"; a data URI base64 antes disso era bloqueada pelo Gmail/Outlook). Ao lado do donut há uma **legenda HTML** (dot colorido + nome + valor bold + %) + barra "Recorrentes vs únicas". Se o QuickChart ou o upload falharem, fallback para barras horizontais "Despesas por categoria" + barra split.
...
- O email inclui um **PDF em anexo** (`relatorio-AAAA-MM.pdf`), gerado com `pdf-lib` (via `esm.sh`, corre nativamente em Deno). **Desde 2026-08-21 o PDF espelha o email**: mesmas secções pela mesma ordem — cabeçalho, estatísticas, despesas do mês, donut à esquerda (~190pt, a mesma imagem PNG do QuickChart via `embedPng`) com **legenda por categoria à direita** (quadrado de cor + nome + valor + %) e barra "Recorrentes vs únicas" — removidas as barras por categoria e a legenda própria que existiam só no PDF.
...
- A mesma cor é usada em **todo o relatório** (email e PDF): swatch/quadrado junto a cada categoria, donut (imagem PNG do QuickChart, que usa as cores da paleta no email e PDF) e legenda; nas barras de fallback (quando o QuickChart falha), largura proporcional ao valor com %.
