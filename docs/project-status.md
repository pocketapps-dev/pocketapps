...
- **Relatório — donut visível em todos os clientes + PDF idêntico ao email** ✅ (2026-08-21): o donut passou a ser um **PNG gerado pelo QuickChart.io e embutido inline via CID** (`<img src="cid:donut-chart">`, parte MIME `inline` do próprio email) — substituiu a data URI base64 (bloqueada pelo Gmail/Outlook → gráfico invisível) e o `conic-gradient` (sem suporte nesses clientes); não aparece como anexo. O **PDF em anexo foi redesenhado para ficar idêntico ao email**: mesmas secções pela mesma ordem (cabeçalho, estatísticas, despesas do mês, donut centrado com a mesma imagem, barra "Recorrentes vs únicas") — removidas as barras por categoria e a legenda própria do PDF. Fallback: se o QuickChart falhar, o email usa barras por categoria (sem donut) e o PDF sai sem imagem.
