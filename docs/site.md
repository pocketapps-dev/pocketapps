...
- **Loja de temas na app (2026-08-04)**: a página `/settings/themes` do PocketExpenses abre `https://pocketapps.pt/themes.html` para comprar os temas pagos — os temas comprados sincronizam via `get_user_themes`. Desde 2026-08-10 tocar num tema premium abre um diálogo na app ("Ver planos" → `/settings/plans` ou "Obter no site" → `themes.html`) — o banner Premium e a secção PREMIUM separada foram removidos (todos os não-gratuitos ficam em "TEMAS PAGOS").
